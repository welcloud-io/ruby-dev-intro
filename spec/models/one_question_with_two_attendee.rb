...
class TestPollQuestionWithTwoResponsesGivenAtDifferenteTimeByTwoDifferentUsers < Test::Unit::TestCase
  ...
  def test01
    expected = { $answer_1 => [$user_id_1],      $answer_2 => [$user_id_2] }
    assert_equal expected, $poll_question.users_for_each_answer
  end

  def test02
    assert_equal 2, $poll_question.total_number_of_users
  end
  ...
  def test04
    assert_equal 50, $poll_question.rate_for($answer_1)
    assert_equal 50, $poll_question.rate_for($answer_2)
  end

  def test05
    expected = { $question_id_1 => { $answer_1 => 50, $answer_2 => 50 } }
